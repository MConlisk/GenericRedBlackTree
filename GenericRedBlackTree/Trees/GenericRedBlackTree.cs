...
public sealed class GenericRedBlackTree<TValue> : IGenericRedBlackTree<TValue>
{
    ...
    private HashSet<int> _index = GenericFactoryPool.GenericFactoryPool.Create(() => new HashSet<int>());
	...
	public void Insert(int key, TValue value)
	{
		...
		if (_index.Add(key))
		{
			var newNode = GenericFactoryPool.GenericFactoryPool.Create(() => new GenericRedBlackTreeNode<TValue>(key, value));
    ...
    public void Remove(int key)
    {
        ...
        GenericFactoryPool.GenericFactoryPool.Recycle(node);
    }
    ...
    {
        var queue = GenericFactoryPool.GenericFactoryPool.Create(() => new Queue<GenericRedBlackTreeNode<TValue>>());
        GenericFactoryPool.GenericFactoryPool.SetPoolResetAction<Queue<GenericRedBlackTreeNode<TValue>>>((queue) => queue.Clear());
        queue.Enqueue(_root);
        ...
        GenericFactoryPool.GenericFactoryPool.Recycle(queue);
    }
    ...
    public IEnumerable<KeyValuePair<int, TValue>> GetAll()
    {
        ...
        while (queue.Count > 0)
        {
            ...
            GenericFactoryPool.GenericFactoryPool.Recycle(currentNode);
        }
    ...
    private static GenericRedBlackTreeNode<TValue> BuildTreeFromArray(KeyValuePair<int, TValue>[] pairsArray, int start, int end, GenericRedBlackTreeNode<TValue> parent)
    {
        ...
        var newNode = GenericFactoryPool.GenericFactoryPool.Create(() => new GenericRedBlackTreeNode<TValue>(pairsArray[mid].Key, pairsArray[mid].Value));
    ...
    public void ResetState()
    {
        ...
        GenericFactoryPool.GenericFactoryPool.Recycle(_index);
        GenericFactoryPool.GenericFactoryPool.Recycle(_root);

        _index = GenericFactoryPool.GenericFactoryPool.Create(() => new HashSet<int>());
        _maxSize = default;
    }
